...
def learn_edge_probs(G, x_V, training_samples, D_g):
    ...
    max_epochs, lr, threshold = 1, (1 / len(training_samples)) ** 0.5, 1e-3
    complete = False
    while not complete and max_epochs > 0:
        random.shuffle(training_samples)
        print('Epochs remaining: ', max_epochs)
        for sample in training_samples: # tqdm(training_samples):
            X, v, y = sample
            # print(sample)
...
def read_graph_from_file(f_name, V):
    G = snap.TNGraph.New()
    for i in range(V):
        G.AddNode(i)
    with open(f_name, 'r') as f:
        for l in f.readlines():
            ...
            G.AddEdge(u, v)
    print('%d nodes loaded' % G.GetNodes())
    print('%d edges loaded' % G.GetEdges())
    return G
...
def main(f_name, V, M):
    ...
    G = read_graph_from_file(f_name, V)
    import pdb; pdb
    x_V = np.array([np.random.rand(D_a) for _ in range(V)])
...
if __name__ == '__main__':
    if len(sys.argv) != 5:
        print('4 arguments needed - graph file of edges, number of nodes V, number of samples M and number of trials')
        exit(0)
    f_name, V, M, trials = sys.argv[1:]
    for seed in range(int(trials)):
        random.seed(seed)
        torch.manual_seed(seed)
        np.random.seed(seed)
        main(f_name, int(V), int(M))